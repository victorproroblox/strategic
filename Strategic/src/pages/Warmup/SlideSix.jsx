...
const CORRECT_RULES = {
  Do: ['I', 'You', 'We', 'They'],
  Does: ['He', 'She', 'It'],
  Did: ['I', 'You', 'We', 'They', 'He', 'She', 'It'] // Did acepta todos
};

const SlideSix = () => {
  const [step, setStep] = useState(1);
  const [activeRoot, setActiveRoot] = useState(null); // 'Do', 'Does' o 'Did'
  
  // Ahora guardamos 2 conexiones por pronombre: { top: 'Do', bottom: 'Did' }
  ...
  const handleConnect = (pronounId) => {
    ...
    // Determinamos si la palabra raíz seleccionada va arriba (Do/Does) o abajo (Did)
    ...
    setConnections(prev => ({
      ...prev,
      [pronounId]: {
        ...(prev[pronounId] || {}), // Mantenemos las conexiones previas del pronombre
        [key]: rootWord // Asignamos la nueva conexión
      }
    }));
    setActiveRoot(null);
    ...
    if (!isCorrect) {
      setTimeout(() => {
        setConnections(prev => {
          ...
          if (!entry || entry[key] !== rootWord) return prev; // ya se corrigió mientras tanto
  ...
  useEffect(() => {
    const drawLines = () => {
      ...
      Object.entries(connections).forEach(([pronounId, conn]) => {
        ...
        // 1. Dibujar línea SUPERIOR (hacia Do o Does)
        if (conn.top && rootRefs.current[conn.top]) {
          ...
          newLines.push({
            ...
            startY: rootRect.bottom - containerRect.top, // Sale de ABAJO de la palabra superior
            endX: (targetRect.left + targetRect.width / 2) - containerRect.left,
            endY: targetRect.top - containerRect.top, // Llega ARRIBA del pronombre
        ...
        if (conn.bottom && rootRefs.current[conn.bottom]) {
          ...
          newLines.push({
            id: `${pronounId}-bottom`,
            startX: (rootRect.left + rootRect.width / 2) - containerRect.left,
            startY: rootRect.top - containerRect.top, // Sale de ARRIBA de "Did"
            endX: (targetRect.left + targetRect.width / 2) - containerRect.left,
            endY: targetRect.bottom - containerRect.top, // Llega ABAJO del pronombre
  ...
  // Función auxiliar para pintar la 'D' inicial de amarillo
